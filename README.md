...
- **소스 저장소**: https://github.com/dyj02056/sktassign4_noticeboard/commit/897cc774133c09b09e53b4a8d03c03758a67b5c2
...
AI 제안을 따르지 않은 일: (진행하며 기록, 없으면 "없음"과 이유)
  1. **고정 키워드 → 동적 Top 5**: AI는 처음에 `AI`/`security`/`LLM` 3개 고정 키워드를 제안했으나, "왜 이 3개인가"라는 근거가 약하고 "IT 최신 근황"과 맞지 않아 동적 Top 5 집계로 전환
  2. **계약서 위치 루트 → `public/`**: AI는 처음에 `public-contract.json`을 저장소 루트에 두자고 했으나, 심사자가 URL로 직접 받아 SHA-256을 검증하려면 `public/`이 낫다고 판단해 이동
  3. **OSV.dev → Hacker News**: AI는 처음에 보안 취약점 데이터(OSV.dev)를 추천했으나, 변동성이 눈에 잘 보이지 않는다는 지적에 따라 Hacker News 키워드 트렌드로 전환
